...
describe("search", () => {
	...
	it("Test 33b: quotes the price a customer will actually be invoiced", () => {
		const quoted = search("id", "is", "FT-110-A", { resident: true })[0];
		const invoiced = priceFor(offering("FT-110-A"), customer("c09"));
		expect(quoted).to.contain("$" + invoiced.toFixed(2));
	});
});

describe("invoice", () => {
	it("Test 34: prices an adult resident", () => {
		expect(priceFor(offering("FT-110-A"), customer("c09"))).to.equal(117.81);
	});

	it("Test 35: adds the non-resident surcharge", () => {
		expect(priceFor(offering("FT-110-A"), customer("c10"))).to.equal(147.26);
	});

	it("Test 36: applies the concession to seniors", () => {
		expect(priceFor(offering("FT-110-A"), customer("c01"))).to.equal(82.47);
	});

	it("Test 37: applies the concession to youth", () => {
		expect(priceFor(offering("FT-110-A"), customer("c20"))).to.equal(82.47);
	});
...
describe("loyalty", () => {
	...
	it("Test 42f: promotes a customer who has taken four", () => {
		expect(customer("c03").interests).to.have.length(4);
		expect(customer("c03").status).to.equal("Community Enthusiast");
	});
});
